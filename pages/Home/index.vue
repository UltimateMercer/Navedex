<template>
  <div>
    <b-navbar
      tag="nav"
      toggleable="md"
      type="light"
      variant="light"
      class="mb-4"
    >
      <div class="container-fluid">
        <b-navbar-brand>
          <nuxt-link to="/" class="navbar-brand py-2">
            <svg
              width="146"
              height="37"
              viewBox="0 0 146 37"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M75.7433 28.2321H80.5084V12.1469H75.7433V13.8375H75.6957C75.1238 12.8231 73.6943 11.5672 71.0735 11.5672C66.8326 11.5672 63.0681 14.9002 63.0681 20.1654C63.0681 24.7542 66.0225 28.7635 71.0735 28.7635C72.9319 28.7635 74.8856 28.0389 75.6957 26.4932H75.7433V28.2321ZM75.7433 20.1649C75.7433 21.7106 74.7426 24.2708 71.7882 24.2708C68.9292 24.2708 67.8332 21.7106 67.8332 20.2132C67.8332 18.3777 69.0245 16.0591 71.7406 16.0591C74.4091 16.0591 75.7433 18.2327 75.7433 20.1649Z"
                fill="#212121"
              />
              <path
                d="M92.6118 28.1838H87.8467L82.4144 12.0985H87.1795L90.2292 22.1458H90.2769L93.4695 12.0985H98.2346L92.6118 28.1838Z"
                fill="#212121"
              />
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M110.767 27.2657C112.101 26.3479 113.245 24.9471 114.007 23.1598V23.1115H109.147C108.479 23.9327 107.717 24.609 106.24 24.609C104.381 24.609 102.809 23.498 102.523 21.5658H114.436C114.439 21.5432 114.442 21.5207 114.445 21.4984C114.49 21.1873 114.531 20.8958 114.531 20.3099C114.531 15.3829 110.957 11.5186 106.145 11.5186C101.284 11.5186 97.8056 15.6727 97.8056 20.165C97.8056 24.9954 101.713 28.7148 106.24 28.7148C107.86 28.7148 109.432 28.2318 110.767 27.2657ZM106.192 15.6241C108.67 15.6241 109.623 17.4113 109.814 18.3774H102.571C102.809 17.4113 103.714 15.6241 106.192 15.6241Z"
                fill="#212121"
              />
              <path
                d="M121.917 12.0982H126.349V13.7888H126.396C126.778 13.016 127.635 11.5186 130.351 11.5186V16.349C128.159 16.3973 126.635 16.8803 126.635 19.2955V28.1352H121.869V12.0982H121.917Z"
                fill="#212121"
              />
              <path
                d="M139.929 16.6392C139.882 16.301 139.834 15.4316 138.452 15.4316C137.452 15.4316 137.118 16.0595 137.118 16.4942C137.118 17.3637 138.309 17.8468 139.548 18.2332C142.217 19.0061 145.123 19.7789 145.123 23.2568C145.123 26.6381 142.169 28.7635 138.643 28.7635C136.26 28.7635 132.591 27.6525 132.162 23.4017H136.88C137.07 24.8992 138.5 24.8992 138.69 24.8992C139.5 24.8992 140.358 24.4161 140.358 23.6433C140.358 22.3873 139.167 22.2424 136.165 21.0831C134.116 20.4069 132.4 19.0544 132.4 16.8807C132.4 13.6926 135.26 11.5672 138.547 11.5672C140.644 11.5672 144.266 12.3884 144.647 16.6392H139.929Z"
                fill="#212121"
              />
              <path
                d="M59.3512 12.9676C58.4935 12.1947 56.8257 11.5667 55.3008 11.5667C52.4894 11.5667 51.87 13.0642 51.3934 13.837H51.3458V12.1464H46.9142V28.2316H51.6794V18.4259C51.6794 17.7014 51.9653 17.0734 52.4418 16.6387C52.4894 16.5904 52.5371 16.5421 52.5847 16.5421C52.6324 16.4938 52.68 16.4455 52.7277 16.4455H52.7753C53.1565 16.2039 53.5854 16.059 54.0619 16.059C54.6814 16.059 55.2055 16.3005 55.6344 16.687C56.1109 17.1217 56.4445 17.7497 56.4445 18.4742V19.1022C56.4445 19.1988 56.4445 19.3437 56.4445 19.4403V28.2316H61.2096V18.1361C61.2096 14.9963 60.2089 13.6921 59.3512 12.9676Z"
                fill="#212121"
              />
              <path
                d="M117.628 28.2315C116.294 28.2315 115.246 27.1688 115.246 25.8163C115.246 24.4638 116.294 23.4011 117.628 23.4011C118.963 23.4011 120.011 24.4638 120.011 25.8163C120.011 27.1205 118.963 28.2315 117.628 28.2315Z"
                fill="#212121"
              />
              <path
                d="M17.7903 27.9156L17.3645 37L25.0294 27.0166L25.8338 19.5409L32.1266 13.2481L37.0473 0L23.7992 4.8734L17.4591 11.1662L9.98338 11.9706L0 19.6355L9.0844 19.2097L17.7903 27.9156Z"
                fill="#212121"
              />
              <path
                d="M9.51005 22.2847L14.7147 27.5366L7.00238 30.3282L9.51005 22.2847Z"
                fill="#212121"
              />
            </svg>
          </nuxt-link>
        </b-navbar-brand>
        <b-navbar-nav class="ml-auto">
          <button
            class="btn btn-light"
            title="Modo noturno"
            @click.prevent="darkMode"
          >
            Sair
          </button>
        </b-navbar-nav>
      </div>
    </b-navbar>

    <div class="container-fluid">
      <div class="mx-lg-3 mx-md-2 mx-1">
        <div class="row">
          <div class="col-6">
            <h1 class="mb-3"><strong>Navers</strong></h1>
          </div>
          <div class="col-6 text-right">
            <button class="btn btn-dark">Adicionar Naver</button>
          </div>
        </div>

        <div class="row">
          <div class="col-lg-3 col-md-4 col-6">
            <div class="card card-flat card-plain mb-3">
              <img
                src="https://i.imgur.com/diuV6oy.jpg"
                class="card-img-top card-img-fit"
                alt="..."
              />
              <div class="my-3">
                <h4 class="mb-1"><strong>Goku</strong></h4>
                <h5 class="mb-3" style="font-weight: 400;">
                  Front-end developer
                </h5>

                <button class="btn btn-light">
                  <font-awesome-icon :icon="['fas', 'trash']" size="lg" />
                </button>
                <button class="btn btn-light">
                  <font-awesome-icon :icon="['fas', 'pen']" size="lg" />
                </button>
              </div>
            </div>
          </div>
          <div class="col-lg-3 col-md-4 col-6">
            <div class="card card-plain mb-3">
              <img
                src="https://i.imgur.com/diuV6oy.jpg"
                class="card-img-top card-img-fit"
                alt="..."
              />
              <div class="my-3">
                <h4 class="card-title"><strong>Goku</strong></h4>
                <h5 class="card-text" style="font-weight: 400;">
                  Front-end developer
                </h5>
                <p class="card-text">
                  Last updated 3 mins ago
                </p>
              </div>
            </div>
          </div>

          <div class="col-lg-3 col-md-4 col-6">
            <div class="card card-plain mb-3">
              <img
                src="https://i.imgur.com/diuV6oy.jpg"
                class="card-img-top card-img-fit"
                alt="..."
              />
              <div class="my-3">
                <h4 class="card-title"><strong>Goku</strong></h4>
                <h5 class="card-text" style="font-weight: 400;">
                  Front-end developer
                </h5>
                <p class="card-text">
                  Last updated 3 mins ago
                </p>
              </div>
            </div>
          </div>
          <div class="col-lg-3 col-md-4 col-6">
            <div class="card card-plain mb-3">
              <img
                src="https://i.imgur.com/diuV6oy.jpg"
                class="card-img-top card-img-fit"
                alt="..."
              />
              <div class="my-3">
                <h4 class="card-title"><strong>Goku</strong></h4>
                <h5 class="card-text" style="font-weight: 400;">
                  Front-end developer
                </h5>
                <p class="card-text">
                  Last updated 3 mins ago
                </p>
              </div>
            </div>
          </div>
          <div class="col-lg-3 col-md-4 col-6"></div>
        </div>
      </div>
    </div>

    <div class="container-fluid">
      <div
        class="col-xl-8 offset-xl-2 col-lg-8 offset-lg-2 col-md-10 offset-md-1 col-12"
      >
        <div class="card card-plain mb-5">
          <div class="card-header">
            <h2>
              <font-awesome-icon :icon="['fas', 'angle-left']" size="lg" />
              <strong> Adicionar Naver </strong>
            </h2>
          </div>
          <div class="card-body">
            <form action="">
              <div class="row">
                <div class="col-lg-6 col-md-6 col-12">
                  <div class="form-group mb-4">
                    <label for="">Nome</label>
                    <input
                      class="form-control border border-dark form-dark"
                      type="text"
                      placeholder="Nome"
                    />
                  </div>
                </div>
                <div class="col-lg-6 col-md-6 col-12">
                  <div class="form-group mb-4">
                    <label for="">Cargo</label>
                    <input
                      class="form-control border border-dark form-dark"
                      type="text"
                      placeholder="Cargo"
                    />
                  </div>
                </div>
                <div class="col-lg-6 col-md-6 col-12">
                  <div class="form-group mb-4">
                    <label for="">Idade</label>
                    <input
                      class="form-control border border-dark form-dark"
                      type="text"
                      placeholder="Idade"
                    />
                  </div>
                </div>
                <div class="col-lg-6 col-md-6 col-12">
                  <div class="form-group mb-4">
                    <label for="">Tempo de empresa</label>
                    <input
                      class="form-control border border-dark form-dark"
                      type="text"
                      placeholder="Tempo de empresa"
                    />
                  </div>
                </div>
                <div class="col-lg-6 col-md-6 col-12">
                  <div class="form-group mb-4">
                    <label for="">Projetos que participou</label>
                    <input
                      class="form-control border border-dark form-dark"
                      type="text"
                      placeholder="Projetos que participou"
                    />
                  </div>
                </div>
                <div class="col-lg-6 col-md-6 col-12">
                  <div class="form-group mb-4">
                    <label for="">URL da foto do Naver</label>
                    <input
                      class="form-control border border-dark form-dark"
                      type="text"
                      placeholder="URL da foto do Naver"
                    />
                  </div>
                </div>
              </div>
              <div class="d-flex justify-content-end">
                <button class="btn btn-dark">Salvar</button>
              </div>
            </form>
          </div>
        </div>
      </div>
    </div>

    <div class="container">
      <div class="card card-horizontal-post mb-5" style="min-height: 500px;">
        <div class="row">
          <div class="col-lg-6 col-12">
            <div
              class="card-horizontal-post-avatar"
              style="
                background-image: url('https://i.imgur.com/diuV6oy.jpg');
                min-height: 500px;
              "
            ></div>
          </div>
          <div class="col-lg-6 col-12">
            <div class="card-body">
              <button class="btn btn-light float-right">
                <font-awesome-icon :icon="['fas', 'times']" size="lg" />
              </button>
              <h4 class="mt-2"><strong>Goku</strong></h4>
              <h5 class="mb-3" style="font-weight: 400;">
                Front-end developer
              </h5>
              <h4 class=""><strong>Idade</strong></h4>
              <h5 class="mb-3" style="font-weight: 400;">
                38
              </h5>
              <h4 class=""><strong>Tempo de empresa</strong></h4>
              <h5 class="mb-3" style="font-weight: 400;">
                Front-end developer
              </h5>
              <h4 class=""><strong>Projeto que participou</strong></h4>
              <h5 class="mb-3" style="font-weight: 400;">
                Front-end developer
              </h5>
              <div class="mt-5">
                <button class="btn btn-light">
                  <font-awesome-icon :icon="['fas', 'trash']" size="lg" />
                </button>
                <button class="btn btn-light">
                  <font-awesome-icon :icon="['fas', 'pen']" size="lg" />
                </button>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>

    <div class="container">
      <div class="col-md-8 offset-md-2 col-12">
        <div class="card card-flat mb-5">
          <div class="card-body py-4 px-5">
            <button class="btn btn-light float-right">
              <font-awesome-icon :icon="['fas', 'times']" size="lg" />
            </button>
            <h2 class="mt-2 mb-3"><strong> Naver criado!</strong></h2>
            <h5 class="mb-4" style="font-weight: 400;">
              Naver criado com sucesso!
            </h5>
          </div>
        </div>
        <div class="card card-flat mb-5">
          <div class="card-body py-4 px-5">
            <button class="btn btn-light float-right">
              <font-awesome-icon :icon="['fas', 'times']" size="lg" />
            </button>
            <h2 class="mt-2 mb-3">
              <strong> Naver atualizado! </strong>
            </h2>
            <h5 class="mb-4" style="font-weight: 400;">
              Naver atualizado com sucesso!
            </h5>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>
<script>
import Vue from 'vue'
import { NavbarPlugin } from 'bootstrap-vue'
Vue.use(NavbarPlugin)
export default {}
</script>
